...
import { ProductImages } from "@/interfaces/product";
...
export const ProductForm = ({ product, categories }: Props) => {
  ...
  const onSubmit = async (data: FormInputs) => {
    ...
    const { ok, productTx } = await createUpdateProduct(formData);
          ...
          <div className="flex flex-wrap">
            {sizes.map((size) => (
              ...
              <button
                ...
                key={size}
                className={clsx(
                  "flex border-none items-center justify-center w-10 h-10 mr-2 border rounded-md",
                  {
            ...
            {product.ProductImage?.map((image) => (
              <div key={image.id}>
                ...
                <button
                  ...
                  className="btn-danger w-full mt-2 rounded-sm shadow-sm"
                >
                  Eliminar
                </button>
              </div>
